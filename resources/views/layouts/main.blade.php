...
<body class="min-h-screen flex flex-col bg-gray-900">
    <header class="flex justify-between p-6 bg-gray-700 shadow-md items-center top-0 z-10">
        ...
        <nav class="flex items-center gap-5">
            @guest
            <a href="{{ route('login') }}" class="text-white px-3 py-1 cursor-pointer rounded hover:border">Login</a>
            <a href="{{ route('registro') }}" class="text-white px-3 py-1 cursor-pointer rounded hover:border">Registrar</a>
            @endguest
            @auth
            @if (auth()->user()->role == 'aluno')
            <div class="flex">
                <a class="text-white px-3 py-1 cursor-pointer rounded border-transparent hover:border" href="{{route('minhasnotas')}}">Minhas Notas</a>
            </div>
            <div class="flex">
                <a class="text-white px-3 py-1 cursor-pointer rounded hover:border" href="{{route('perfil.edit', auth()->user()->id)}}">Editar Perfil</a>
            </div>
            @else
            <div class="flex">
                <a class="text-white px-3 py-1 cursor-pointer rounded hover:border" href="{{route('listarnotas')}}">Listar Notas</a>
            </div>
            <div class="flex">
                <a class="text-white px-3 py-1 cursor-pointer rounded hover:border" href="{{route('aluno.index')}}">Listar Alunos</a>
            </div>
            @endif
            <form action="/logout" method="post">
                @csrf
                <a class="text-white px-3 py-1 cursor-pointer rounded hover:border" href="{{route('logout')}}" onclick="event.preventDefault();this.closest('form').submit()">Sair</a>
            </form>
            @endauth
        </nav>
    </header>
    ...
    <footer class="bg-gray-700 shadow-md p-5 text-center text-gray-100">
        &copy; Todos os direitos reservados 2024. <b>Grupo: 4.</b> Faculdade de Tecnologia de Jahu - Curso DSM
    </footer>
    <script>
